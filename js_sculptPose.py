...
def run (scu, geo, mode, lv = 1, ihi = 1):
	...
	for i in vtxs:
		vtxId = i.split ('[')[1]
		vtxId = vtxId.split (']')[0]
		geoPos = cmds.pointPosition ('{}.vtx[{}]'.format (geo,vtxId))
		scuPos = cmds.pointPosition ('{}.vtx[{}]'.format (scu,vtxId))
...
def ui ():
	...
	cmds.iconTextButton (l = '                        Sculpt', i = 'putty.png', bgc = bColor_green, ann = 'Duplicate selected geo for sculpting.', st = 'iconAndTextHorizontal', w = bWidth, h = bHeight, c = 'import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_sculpt ()')
	cmds.button (l = 'Sculpt From Zero', bgc = bColor_green, ann = 'Duplicate selected geo for sculpting, with all existing sculpt deformation removed.', w = bWidth, h = bHeight2, c = 'import js_sculptPose ; reload (js_sculptPose) ; js_sculptPose.ui_sculptFromZero ()')
	cmds.separator (h = 6, style = 'double')
	cmds.iconTextButton (l = '                 Apply Standard', bgc = bColor_blue, i = 'nurbsToPolygons.png', st = 'iconAndTextHorizontal', ann = 'Apply selected sculpt as a standard shape.', h = bHeight, c = 'import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_apply ()')
	cmds.button (l = 'Apply Standard 1-Frame', bgc = bColor_blue, ann = 'Apply selected sculpt as a standard shape keyed 0-1-0 around current frame.', h = bHeight2, c = 'import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_apply_1f ()')
	cmds.separator (h = 6, style = 'double')
	cmds.iconTextButton (l = '             Apply Pose-To-Pose', bgc = bColor_purple, i = 'setKeyOnAnim.png', st = 'iconAndTextHorizontal', ann = 'Apply selected sculpt as a Pose-To-Pose shape, automatically keying in and out of existing P2P shapes.', h = bHeight, c = 'import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_apply_p2p ()')
	cmds.button (l = 'Create P2P Zero', bgc = bColor_purple, ann = 'Skipping the sculpt step, create a P2P shape with no sculpt deformation.', w = (bWidth/2), h = bHeight2, c = 'import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_createP2PZero ()')
	cmds.button (l = 'Create P2P Hold', bgc = bColor_purple, ann = 'Skipping the sculpt step, create a P2P shape with the deformation of the previous P2P sculpt.', w = (bWidth/2), h = bHeight2, c = 'import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_createP2PHold ()')
	cmds.separator (h = 6, style = 'double')
	cmds.rowColumnLayout (nc = 2, rs = [2,3], cs = [2,3])
	cmds.button (l = 'Delete CB Sel', bgc = bColor_red, ann = 'Deletes shapes and releated nodes based on channel box selection.', w = (bWidth/2), h = bHeight2, c = 'import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_delete (allP2P = 0)')
	cmds.button (l = 'Delete All P2P', bgc = bColor_red, ann = 'Deletes entire Pose-To-Pose System.', w = (bWidth/2), h = bHeight2, c = "import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_delete (allP2P = 1)")
	cmds.setParent ('..')
	cmds.separator (h = 6, style = 'double')
	cmds.rowColumnLayout (nc = 2, rs = [2,3], cs = [2,3])
	cmds.button (l = 'Edit Sculpt', bgc = bColor_brown, ann = 'Start a sculpt that will be automatically driven by the selected sculpt in the channel box.', w = bWidth/2, h = bHeight2, c = 'import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_editSculpt ()')
	cmds.button (l = 'Apply Edit', bgc = bColor_brown, ann = 'Apply the edited sculpt.', w = bWidth/2, h = bHeight2, c = 'import js_sculptPose ; reload(js_sculptPose) ; js_sculptPose.ui_applyEdit ()')
	cmds.setParent ('..')
	cmds.button (l = 'Sort Channel Box', bgc = bColor_brown, h = bHeight2, ann = "If you used undo after hitting one of the delete buttons, you may need to sort the channel box with this button.", c = "import js_sculptPose ; reload (js_sculptPose) ; js_sculptPose.sortCB ()")
	cmds.separator (h = 6, style = 'double')
